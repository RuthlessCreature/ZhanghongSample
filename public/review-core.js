...
function rooms(text=''){
  ...
  let m;
  while((m=re.exec(t))){
    const name=norm(m[2]).replace(/\s+(SHEET|REV|ISSUE)$/,'').trim();
...
function unresolvedMentions(text=''){
  const src=norm(text), up=src.toUpperCase(), out=[];
  ...
  while((m=re.exec(up))){
    ...
    const excerpt=src.slice(start,end).trim(),e=excerpt.toUpperCase();
    if(/ALL\s+CHECK.{0,80}SHALL\s+BE\s+CLOSED/.test(e))continue;
    if(/NO\s+(CHECK|VERIFY|TBD|TBC|PENDING|HOLD).{0,60}(MAY|SHALL)/.test(e))continue;
    if(/(CHECK|VERIFY|TBD|TBC|PENDING|HOLD).{0,60}(NOT\s+PERMITTED|PROHIBITED)/.test(e))continue;
...
export function deterministicReview(pages=[],sourcePages=pages.length){
  const normalized=pages.map((p,i)=>{
    ...
    return {
      page:Number(p.pageNumber||i+1),sheetId,text,role:classifySheet(text),
      windows:marks(text,'W'),doors:marks(text,'D'),refs:referencedSheets(text),
      rooms:rooms(text),unresolved:unresolvedMentions(text),
      revisionPlaceholders:revisionPlaceholders(text),
      textItemCount:Number(p.textItemCount??p.textItems?.length??0)
    };
  });

  const alerts=[];let seq=1;const aid=()=> 'P'+String(seq++).padStart(3,'0');

  normalized.forEach(p=>{
    if(!p.sheetId)alerts.push(alert(aid(),'medium','图号/图框','第 '+p.page+' 页','未识别到明确图号','PDF 文字层未识别到 A-101 / A-201 等图号；建议人工确认图框。'));
  });
  ...
  for(const [sid,pagesFound] of bySheet){
    if(pagesFound.length>1)alerts.push(alert(aid(),'high','图号/图框',sid,'图号 '+sid+' 在当前文件中重复出现','出现页码：'+pagesFound.join(', ')));
  }
  ...
  if(indexPages.length){
    ...
    if(listedMissing.length)alerts.push(alert(aid(),'high','图纸目录',indexPages.map(pageLocation).join(', '),'图纸目录列出当前套图中不存在的图号','目录列出：'+listedMissing.join(', ')+'；需确认是否漏页、误列或未发行。'));
    if(actualOmitted.length)alerts.push(alert(aid(),'medium','图纸目录',indexPages.map(pageLocation).join(', '),'当前套图存在未列入图纸目录的已发行图号','未列入目录：'+actualOmitted.join(', ')+'。'));
  }
  ...
  if(schedulePages.length){
    ...
    if(fullyLoaded){
      for(const m of scheduleWindows.filter(x=>!planWindows.includes(x))){
        alerts.push(alert(aid(),'medium','门窗一致性',schedulePages.map(pageLocation).join(', '),'门窗表存在窗号 '+m+'，但当前已扫描平面未检出该编号','可能是表内孤立编号、其他楼层图纸缺失或文字提取遗漏；需人工确认。','pdf_text',{mark:m,kind:'window',direction:'schedule-to-plan'}));
      }
      for(const m of scheduleDoors.filter(x=>!planDoors.includes(x))){
        alerts.push(alert(aid(),'medium','门窗一致性',schedulePages.map(pageLocation).join(', '),'门窗表存在门号 '+m+'，但当前已扫描平面未检出该编号','可能是表内孤立编号、其他楼层图纸缺失或文字提取遗漏；需人工确认。','pdf_text',{mark:m,kind:'door',direction:'schedule-to-plan'}));
      }
    }
  }
  ...
  for(const [roomId,refs] of roomMap){
    ...
    if(names.length>1)alerts.push(alert(aid(),'high','房间编号/名称',refs.map(x=>x.sheetId||('P'+x.page)).join(', '),'同一房间编号 ROOM '+roomId+' 出现多个名称','检测到：'+names.join(' ↔ ')+'。','pdf_text',{roomId,names}));
  }

  normalized.forEach(p=>{
    if(p.unresolved.length)alerts.push(alert(aid(),'medium','未闭环标记',pageLocation(p),'发现待确认/待定类文字，正式出图前应闭环',p.unresolved.map(x=>x.keyword+': '+x.excerpt).join(' | ').slice(0,1200),'pdf_text',{keywords:uniq(p.unresolved.map(x=>x.keyword))}));
    if(p.revisionPlaceholders.length)alerts.push(alert(aid(),'medium','图号/图框',pageLocation(p),'修订栏存在占位符或未确认版本号','命中：'+p.revisionPlaceholders.join(', ')));
  });

  if(fullyLoaded){
    normalized.filter(p=>p.role!=='index').forEach(p=>{
      const missing=p.refs.filter(ref=>ref!==p.sheetId&&!loadedSet.has(ref));
      if(missing.length)alerts.push(alert(aid(),'medium','图纸引用',pageLocation(p),'检测到当前套图中未找到的图号引用','引用：'+missing.slice(0,12).join(', ')+'。可能是漏页、未发行图纸或其他专业引用，需人工确认。','pdf_text',{missingRefs:missing}));
    });
  }
  ...
  return {
    textCoverage:{mode:coverageMode,totalTextItems:itemCount,pagesWithText:textPages,pagesWithSheetId:normalized.filter(p=>p.sheetId).length,scannedPages:normalized.length,sourcePages},
    sheets:normalized.map(({text,...x})=>x),
    alerts,
    indices:{planWindows,planDoors,scheduleWindows,scheduleDoors,roomIds:[...roomMap.keys()],actualSheets,indexSheets:indexPages.length?uniq(indexPages.flatMap(p=>p.refs)):[]},
    fullyLoaded
  };
}
...
export function rankReviewPages(pages=[],deterministic,maxPages=10){
  ...
  }).sort((a,b)=>b.score-a.score||a.pageNumber-b.pageNumber).slice(0,maxPages);
}
